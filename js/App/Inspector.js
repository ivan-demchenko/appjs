...
App.Inspector = (function ($) {
	...
	function _inspectElement(element, params) {
		...
		case 'form': {
				var h = new App.Ui.FormHandler($(element), params || {});
				h.Init();
		...
		case 'datePicker': {
				var h = new App.Ui.DatePicker($(element), params || {});
				h.init();
		...
		case 'tabs': {
				var h = new App.Ui.Tabs($(element), params || {});
				h.init();
		...
		case 'autocomplete': {
				var h = new App.Ui.Input.Autocomplete($(element), params || {});
				h.init();
		...
		case 'slider': {
				var h = new App.Ui.Slider($(element), params || {});
				h.init();
		...
		case 'inputNumeric':
		case 'inputMoney': {
				var h = new App.Ui.Input.Numeric($(element), params);
				h.init();
				if (App.Collection.Inputs[$(element).attr('id')] == undefined) {
					App.Collection.Inputs[$(element).attr('id')] = h;
				} else {
		...
		case 'inputTime': {
				var h = new App.Ui.Input.Time($(element), params);
				h.init();
				if (App.Collection.Inputs[$(element).attr('id')] == undefined) {
					App.Collection.Inputs[$(element).attr('id')] = h;
				} else {
		...
		case 'inputMasked': {
				var h = new App.Ui.Input.Masked($(element), params);
				h.init();
				if (App.Collection.Inputs[$(element).attr('id')] == undefined) {
					App.Collection.Inputs[$(element).attr('id')] = h;
				} else {
