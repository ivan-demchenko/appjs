	...
	App.UI.Library = {
		...
		slider : function (element, params) {
			...
			return {
				...
				SlideLimit : function (val) {
					if (val === undefined && _slideLimit != false) {
						return _slideLimit 
					} else {
						_slideLimit = val;
						$(_element).bind("slide", function(e, ui){
							if($(_element).slider("option", "value") > _slideLimit) {
								$(_element).slider("option", "value", _slideLimit);
								return false;
							}
						});
						return this;
				...
				SetEventHandler : function (event, callback) {
